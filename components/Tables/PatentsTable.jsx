...
export default function PatentsTable() {
  ...
  const [metadata, setMetadata] = useState({ total: 0, totalPages: 1, page: 1 });
                        ...
                        <tr key={p.id} className="border-b hover:bg-gray-50">
                          <td className="px-4 py-2">
                            <Checkbox.Root
                              ...
                              onCheckedChange={() => handleRowSelect(p.id)}
                              className="h-4 w-4 border rounded"
                            >
                              <Checkbox.Indicator />
                            </Checkbox.Root>
                          ...
                          <td className="px-4 py-2 max-w-xs truncate" title={p.abstract}>
                            {p.abstract}
                          </td>
                          <td className="px-4 py-2">{formatDate(p.filing_date)}</td>
                          <td className="px-4 py-2">{formatDate(p.grant_date)}</td>
            ...
            <Dialog.Portal>
              ...
              <Dialog.Content className="fixed top-1/2 left-1/2 bg-white rounded-lg p-6 shadow-lg transform -translate-x-1/2 -translate-y-1/2">
                ...
                <Dialog.Description className="text-sm text-gray-600 mt-2">
                  Are you sure you want to delete {selectedRows.length} patent(s)?
                </Dialog.Description>
